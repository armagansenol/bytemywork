...
import { folder, useDevControls } from "@/hooks/use-dev-controls"
...
function CameraController({ camX, camY, camZ, fov }: { camX: number; camY: number; camZ: number; fov: number }) {
  ...
  useFrame(() => {
    // Initialize smooth pointer with current pointer position to prevent initial jump
    if (!initialized.current) {
      smoothPointer.current.x = pointer.x
      smoothPointer.current.y = pointer.y
...
}: // setIsSphereActive,
{
  ...
  useFrame(() => {
    ...
    // Always apply rotation to ensure initial state is correct
...
export function BarrelCarousel() {
  ...
  const currentSlide = useSlideTracking(
    ITEMS.length,
    () => (scrollTriggerReady.current ? cylinderProgressRef.current : 0),
    controls["trigger offset"] as number
  )

  // Extract values from controls with type assertions
  const params = {
    radius: controls.radius as number,
    gapPercent: controls["gap %"] as number,
    heightScale: controls["height scale"] as number,
    camX: controls["cam x"] as number,
    camY: controls["cam y"] as number,
    camZ: controls["cam z"] as number,
    fov: controls.fov as number,
    rotXDeg: controls["rot x"] as number,
    rotYDeg: controls["rot y"] as number,
    rotZDeg: controls["rot z"] as number,
    posX: controls["pos x"] as number,
    posY: controls["pos y"] as number,
    posZ: controls["pos z"] as number,
  }

  const barrel = {
    k: controls.amount as number,
    power: controls.power as number,
  }
  ...
  useGSAP(() => {
    ...
    ScrollTrigger.create({
      ...
      onRefresh: () => {
        // Mark ScrollTrigger as ready after first refresh
  ...
  return (
    <div
      ref={sectionRef}
      className={cn(
        "w-screen h-[60vw] relative",
        "before:w-full before:h-[150px] before:bg-gradient-to-b before:from-black before:to-black/0 before:z-50 before:absolute before:top-0 before:left-0 before:pointer-events-none",
        "after:w-full after:h-[150px] after:bg-gradient-to-t after:from-black after:to-black/0 after:z-50 after:absolute after:bottom-0 after:left-0 after:pointer-events-none"
      )}
    >
      <Canvas>
        ...
        <Suspense fallback={null}>
          <CylinderPreview
            ...
            getScrollProgress={() => (scrollTriggerReady.current ? cylinderProgressRef.current : 0)}
            barrelK={barrel.k}
            ...
            posY={params.posY}
            posZ={params.posZ}
            wireframeEnabled={controls.enabled as boolean}
            wireframeColor={controls.color as string}
            wireframeOpacity={controls.opacity as number}
        ...
        <Html fullscreen pointerEvents='auto'>
          <Link href={ITEMS[currentSlide].link} className='absolute top-64 left-48 z-50'>
            <AnimatePresence mode='wait'>
              ...
              >
                <div className='text-white xl:text-2xl 2xl:text-3xl font-bold group-hover:text-amber-200 transition-colors'>
                  {ITEMS[currentSlide].title}
                </div>
                <div className='text-white xl:text-sm 2xl:text-base font-bold flex items-center gap-2 group-hover:text-amber-200 transition-colors'>
